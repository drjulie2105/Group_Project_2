var container = d3.select('#d3Id'),
      width = 1300,
      height = 660,
      margin = {top: 200, right: 30, bottom: 30, left: 55},
      barPadding = .2,
...
d3.csv("data/merge_df_2000.csv").then(function(electionData) {
  ...
  //   // Cast the candidate votes value to a number for each piece of electionData
...
var state_po_Dem = svg.selectAll(".state_po_Dem")
  .data(electionData)
  .enter().append("g")
  .attr("class", "state_po_Dem")
  .attr("transform", d => `translate(${xScale0(d.state_po_Dem)},0)`);

/* Add field1 bars */
state_po_Dem.selectAll(".bar.candidatevotes_Dem")
  .data(d => [d])
  .enter()
  .append("rect")
...
/* Add field2 bars */
state_po_Dem.selectAll(".bar.candidatevotes_Rep")
  .data(d => [d])
  .enter()
  .append("rect")
